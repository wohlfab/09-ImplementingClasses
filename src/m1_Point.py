...
# -----------------------------------------------------------------------------
# TDONE: 2.          With your instructor, READ THE INSTRUCTIONS in
...
class Point(object):
    ...
    def move_by(self, dx, dy):
        self.distance = self.distance + math.sqrt((self.x - (self.x + dx)) ** 2 + (self.y - (self.y + dy)) ** 2)
        self.x = self.x + dx
        self.y = self.y + dy
        self.moves = self.moves + 1
    ...
    def get_distance_from_start(self):
        return math.sqrt((self.x - self.intx)**2 + (self.y - self.inty)**2)
